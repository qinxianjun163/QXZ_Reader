...
+ (sqlite3 *)open {
    ...
    NSString *sqlPath = [doc stringByAppendingPathComponent:@"audioMovie.sqlite"];
    NSFileManager *fm = [NSFileManager defaultManager];
    //如果指定路径下没有文件
    if (![fm fileExistsAtPath:sqlPath]) {
        NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"audioMovie" ofType:@"sqlite"];
        //我们从bundle中copy过去
...
+ (void)deleteDownloadComplated:(NSString *)url {
    ...
    NSFileManager *fm = [NSFileManager defaultManager];
    //拿到我们的save地址,进行删除
    CategoaryModel *model = [DB findDownloadComplated:url];
...
+ (void)deleteDownloading:(NSString *)url {
    ...
    CategoaryModel *model = [DB findDownloading:url];
    NSFileManager *fm = [NSFileManager defaultManager];
...
+ (CategoaryModel *)findDownloading:(NSString *)url {
    ...
    int result = sqlite3_prepare(db, [sql UTF8String], -1, &stmt, nil);
    //创建一个model
    CategoaryModel *model = nil;
    if (result == SQLITE_OK) {
        //如果sql语句执行成功。初始化我们的model
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            model = [CategoaryModel new];
            const unsigned char *cCategory = sqlite3_column_text(stmt, 0);
            NSString *category = [NSString stringWithUTF8String:(const char *)cCategory];
...
+ (NSArray *)allMovieCollection {
    ...
    sqlite3 *db = [DB open];
    sqlite3_stmt *stmt = nil;
    int result = sqlite3_prepare_v2(db, "select * from collection order by time desc", -1, &stmt, nil);
    //创建array。保存查询出来的数据
    NSMutableArray *array = nil;
    if (result == SQLITE_OK) {
        //如果sql语句成功，初始化数组
...
+ (void)insertMovieCollection:(CategoaryModel *)model
{
    ...
    int time = [[NSDate date] timeIntervalSince1970];
    //创建我们的sql语句
    NSString *sql = [NSString stringWithFormat:@"insert into collection values('%@','%@','%@',%f,'%@',%d,'%@','%@','%@',%d,'%@',%d)",model.category,model.coverBlurred,model.coverForDetail,model.date,model.myDescription,(int)model.duration,model.playUrl,model.title,model.webUrl,(int)model.progress,model.dataPath,time];
    //执行我们的sql语句
    sqlite3_exec(db, [sql UTF8String], nil, nil, nil);
}
...
+ (void)deleteMovieCollection:(NSString *)url {
    sqlite3 *db = [DB open];
    NSString *sql = [NSString stringWithFormat:@"delete from collection where playUrl = '%@'",url];
    sqlite3_exec(db, [sql UTF8String], nil, nil, nil);
}
...
+ (CategoaryModel *)findMovieCollection:(NSString *)url {
    sqlite3 *db = [DB open];
    sqlite3_stmt *stmt = nil;
    NSString *sql = [NSString stringWithFormat:@"select * from collection where playUrl = '%@'",url];
    int result = sqlite3_prepare(db, [sql UTF8String], -1, &stmt, nil);
    //创建一个model
    CategoaryModel *model = nil;
    if (result == SQLITE_OK) {
